...
git -C "$REPOS_DIR/mb" config --local merge.npm-packagejson-keep-ours-jpath-merge-driver.driver 'keep_ours_paths_merge_driver.pyz -t JSON -O %O -A %A -B %B -P ./%P -p version dependencies:@mycompany/.+'
...
git -C "$REPOS_DIR/td" config --local merge.npm-packagejson-keep-ours-jpath-merge-driver.driver 'keep_ours_paths_merge_driver.pyz -t JSON -O %O -A %A -B %B -P ./%P -p version dependencies:@mycompany/.+'
