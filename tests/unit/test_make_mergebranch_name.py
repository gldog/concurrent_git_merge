...
class Test03(unittest.TestCase):
    ...
    def test_valid_placeholders(self):
        repo_metadata = {
            ...
            'source_ref': 'origin/SOURCE-REF',
            'dest_branch': 'DEST-BRANCH'
        }

        merge_branch_template = 'merge/from_{{source_ref.replace("origin/","")}}' \
                                '_into_{{dest_branch}}'
        merge_branch = main.make_mergebranch_name(merge_branch_template, repo_metadata)
        expected_merge_branch = 'merge/from_SOURCE-REF_into_DEST-BRANCH'
        self.assertEqual(expected_merge_branch, merge_branch)

        merge_branch_template = 'merge/from_{{source_ref.replace("origin/","")}}' \
                                '_into_{{dest_branch}}_{{task_start.strftime(("%b%d"))}}'
        merge_branch = main.make_mergebranch_name(merge_branch_template, repo_metadata)
        expected_merge_branch = 'merge/from_SOURCE-REF_into_DEST-BRANCH_Jan01'
        self.assertEqual(expected_merge_branch, merge_branch)
